...
func TestCreateDB(t *testing.T) {
	...
	_, err := postgres.Client.Exec(create_db)
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestPopulateDB(t *testing.T) {
	postgres := Postgres{Client: db}
	_, err := postgres.Client.Exec(populate_db)
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestCheckGoods(t *testing.T) {
	postgres := Postgres{Client: db}
	goods, err := postgres.CheckGoods(1)
	if err != nil {
		t.Fatal(err)
	}
	if len(goods) < 1 {
